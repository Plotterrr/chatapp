...
public class SendPrivateMsgCmd implements ICmd {
    private final Lobby lobby;

    public SendPrivateMsgCmd() {
    ...
    public void execute(Session session, Map<String, String> body) {
        ...
        User privateReceiver = lobby.getUserMap().get(Integer.parseInt(body.get("privateReceiver")));
        String content = body.get("content");
        try {
            if (room.isInRoom(user)) {

                Message message = MessageFactory.getInstance().make(user, privateReceiver, body.get("content"), body.get("messageType")); // sendTo?
